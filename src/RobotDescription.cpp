...
bool RobotDescription::loadURDFAndSRDF(const std::string &URDF_path,
	...
	if (!_models_loaded) {
		...
		for (unsigned int i = 0; i < _xbotcore_model.get_chain_names().size();
				++i) {
			std::cout << "chain #" << i << " "
					<< _xbotcore_model.get_chain_names()[i] << std::endl;
...
bool RobotDescription::initialize(const std::string &URDF_path,
		const std::string &SRDF_path) {
	...
	return parseCollisionObjects(robot_model, my_tree, _xbotcore_model,
			_active_kinematic_chain_name, _link_names);
}

bool RobotDescription::parseCollisionObjects(
		std::shared_ptr<urdf::Model> robot_model, const KDL::Tree &tree_,
	...
	for (unsigned int i = 0; i < link_names.size(); ++i) {
		...
		if (link->collision) {
			...
			std::cout << "Collision for link: " << link->name << " is of type "
					<< link->collision->geometry->type << std::endl;
			...
			if (link->collision->geometry->type == urdf::Geometry::MESH) {
				...
				if (found != std::string::npos) {
					cerr << "Couldn't find mesh for link " << link->name
							<< ". String is malformed and uses ROS terminology (package://). Use gazebo conform (model://) syntax in "
							<< collFileName << endl;
					return false;
				}
				...
				if (resultingPath == "") {
					cerr << "Mesh path is empty for link " << link->name
							<< endl;
					return false;
				}
				...
				if (!boost::filesystem::exists(resultPath)) {
					cerr << "Mesh " << resultingPath
							<< " does not exist for link " << link->name
							<< endl;
					return false;
				}
				...
				for (aiMesh* entry : meshEntries) {
					...
					if (entry->HasPositions()) {
						...
						std::cout << "Vertices loaded! " << verticeCnt
								<< std::endl;
					}
...
//					model->computeLocalAABB();
			...
			} else if (link->collision->geometry->type
...
//
//				std::cout << "collision origin for " << link->name << "="
...
//						<< "l: " << collisionGeometry->length << std::endl;
			...
			// Transform collision object now by Transformation from URDF!
			...
			std::cout << " p[" << position.x << " " << position.y << " "
					<< position.z << "] r[" << r << " " << p << " " << y
					<< "] op[" << link->collision->origin.position.x << " "
					<< link->collision->origin.position.y << " "
					<< link->collision->origin.position.z << "] or[" << por
					<< " " << pop << " " << poy << "]" << std::endl;

			fcl::Transform3d out;
			out.setIdentity();
			fcl::Vector3d t(link->collision->origin.position.x,
					link->collision->origin.position.y,
					link->collision->origin.position.z);
			out.translate(t);
			fcl::Quaterniond q(pqw, pqx, pqy, pqz);
			std::cout << "Quaterniond: pqw = " << pqw << ", pqx = " << pqx
					<< ", pqy = " << pqy << ", pqz = " << pqz << std::endl;
			out.rotate(q);
			...
			std::cout << "Quaterniond2: pqw2 = " << pqw2 << ", pqx2 = " << pqx2
			...
			cout << "Loading collision geometry for link (" << link->name
...
void RobotDescription::get_tree_segment_names_recursive(
...
void RobotDescription::get_tree_segment_names(Tree tree,
		std::vector<std::string> &link_names, bool non_fixed_joints) {
...
/** ####### REAL-TIME CRITICAL PART (START) ####### */
bool RobotDescription::transformAllLinkCollisionObjects(
		const KDL::JntArray &joints_kdl) {
...
//			std::cout << "check! links [" << i << "] = " << _link_names[i] << std::endl;
		std::shared_ptr<fcl::CollisionObjectd> collObj =
				_collision_objects[_link_names[i]];
		if (collObj) {
...
//				std::cout << "old RotationBASE:\n" << baseIteration.rotation() << std::endl;
			...
			Eigen::AngleAxis<double> axisTransform(angleValue,
					Eigen::Vector3d(_joint_axis[_link_names[i]].x,
							_joint_axis[_link_names[i]].y,
							_joint_axis[_link_names[i]].z));
...
//				result.rotate(_origin_transforms[_link_names[i]].rotation());
			collObj->setTransform(
					baseIteration * _origin_transforms[_link_names[i]]);
//
...
//				std::cout << "new RotationBASE:\n" << baseIteration.rotation() << std::endl;
			...
			std::cerr << "Collision object for link " << _link_names[i]
					<< " not found!" << std::endl;
		}
	}
...
std::shared_ptr<fcl::CollisionObjectd> RobotDescription::getCollisionObjectForLink(
